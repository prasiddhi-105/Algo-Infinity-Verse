...
export function getTopicName(topicKey) {
  const topicMeta = (globalThis.dsaTopics || []).find(
    (topic) => topic.name?.toLowerCase() === topicKey || topic.id?.toString() === topicKey
  );
...
export function getTopicProblemCount(topicKey) {
  ...
  return globalThis.practiceProblems.filter(
    (problem) => problem.category?.toLowerCase() === topicKey.toLowerCase()
  ).length;
}
...
export function buildReason(progress, topicKey) {
  ...
  if (topicProblems > 0 && completedCount < topicProblems)
    return '📚 You have prior learning history for this topic and should reinforce it.';
...
export function toggleRevisionTaskCompletion(taskId, progress = {}) {
  const state = normalizeProgress(progress);
  const { tasks } = buildRevisionTasks(state);
