...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initializeGreedyPage);
} 
...
function initNavbar() {
    ...
    window.addEventListener("scroll", () => {
        ...
        navbar.style.background = window.scrollY > 100 ? "rgba(10, 10, 26, 0.95)" : "rgba(10, 10, 26, 0.8)";
    });
}
...
function initHeroTyping() {
    const el = document.getElementById("typingTextGreedy");
    if (!el) return;
...
function initExerciseToggles() {
    document.querySelectorAll(".greedy-exercise-toggle").forEach((btn) => {
...
function initCopyButtons() {
    document.querySelectorAll(".greedy-code-copy").forEach((btn) => {
...
function initSidebarSpy() {
    const links = document.querySelectorAll(".greedy-sidebar-nav a");
    const lessons = document.querySelectorAll(".greedy-lesson");
...
function initProgressTracker() {
  const STORAGE_KEY = "greedy-learning-progress";
  const TOTAL_TOPICS = 8;
  const fill = document.getElementById("progressFill");
  const count = document.getElementById("progressCount");
  const bar = document.querySelector(".greedy-progress-bar");
...
function initQuiz() {
  ...
  const optionCards = document.querySelectorAll(".greedy-quiz-option");
  optionCards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      const parentOptions = document.querySelectorAll(
        `.greedy-quiz-option input[name="${name}"]`,
      );
      parentOptions.forEach((o) => {
        o.closest(".greedy-quiz-option").classList.remove("selected");
      });
  ...
  if (submitBtn) {
    submitBtn.addEventListener("click", () => {
      ...
      for (let key in correctAnswers) {
        ...
        radios.forEach((r) => {
          r.disabled = true;
          const card = r.closest(".greedy-quiz-option");
        ...
        if (selectedRadio.value === correctVal) {
          ...
          if (feedback) {
            ...
            feedback.className = "greedy-quiz-feedback correct";
          }
        } else {
          if (feedback) {
            feedback.textContent = "✗ Incorrect Answer.";
            feedback.className = "greedy-quiz-feedback incorrect";
          }
        }
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", () => {
      const radios = document.querySelectorAll(
        '.greedy-quiz-option input[type="radio"]',
      );
      radios.forEach((r) => {
        r.checked = false;
        r.disabled = false;
        const card = r.closest(".greedy-quiz-option");
        card.className = "greedy-quiz-option";
      });

      const feedbacks = document.querySelectorAll(".greedy-quiz-feedback");
      feedbacks.forEach((f) => {
        f.textContent = "";
        f.className = "greedy-quiz-feedback";
      });

      const explanations = document.querySelectorAll(".greedy-quiz-explanation");
